...
- **Haiku**: 単純な調査・短いファイル生成・handoff.mdの更新
- **Sonnet**: 楽曲仕様書・歌詞草稿・SNS文案・プレスリリース（デフォルト）
- **Opus**: SVPファイル生成・複雑なコード生成・**重要な設計判断**・品質が特に重要な判断
  ※ **Opus 4.7**（`claude-opus-4-7`）を使用。価格は要確認（旧 Opus 4.6: $5/$25/MTok）
- Agent Teams起動時は全員Sonnetで開始し、必要に応じてOpusに切り替える
