...
- **Ichi**: 咲かせる（実装済み・`website/flora/hana.js`）— Hare「筆あそび」の対称版。花図鑑 cover の「咲かせて遊ぶ」CTA から全画面ビューへ入り、指でなぞった軌跡に手続き的な花（桜/菊/チューリップ/小花の4形・Canvas 2D）が「つぼみ→満開」で芽吹き、庭が育つ。季節パレット4種（春淡紅/夏水青/秋琥珀/冬白銀）で配色を切替、なぞる速度で花の大きさが変わる（fude の速度依存線幅の対称）。咲いた庭は「押し花にする」で和紙地に平面化・退色・季節ラベルを添えた PNG に保存。reduced-motion で即満開・タッチ対応・花総数上限400。設計/計画: `docs/superpowers/{specs,plans}/2026-07-06-ichi-sakaseru*.md`。Phase 2a（実装済み・2026-07-07「チープさ」解消と瞑想性向上）: 花びらをグラデーション＋半透明＋花びら毎ゆらぎに（脱ベタ塗り）、季節連動の暗い庭の光だまり背景（`.hana-stage` CSSに分離しpress()は透明花のまま）、奥行き（depthで奥は小さく淡く柔らか）、低opacityグレイン、開花後の微風（常時rAF＋30fpsスロットル＋sway・reduceで停止）、遅さの報酬化（ゆっくりなぞるほど大きく豊かに・速いとまばら）、呼吸のテンポ（開花約1.5秒）、入退場の儀式（入場=中央に招待花＋フェードイン／退場=花が閉じてから暗転 farewell）。設計/計画: `docs/superpowers/{specs,plans}/2026-07-07-ichi-phase2a*.md`。Phase 2b（次期）: 散る花（無常）・図鑑側の余白/季節連動。Phase 2c/バックログ: なぞり音（Web Audio生成）・一輪挿し/絵本。
