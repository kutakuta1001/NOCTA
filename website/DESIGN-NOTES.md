...
- **Gyu**: 石を光にかざす（実装済み・`website/gems/kazasu.js` + `website/gems/gem3d.js`）— Phase 1で写真＋擬似光沢、Phase 2でコード生成の本物の3D宝石メッシュ（Three.js）へ。ライトボックス内でPhoto/Gemトグル。3Dジェムは58面級ブリリアントカット等のプロシージャルジオメトリ＋MeshPhysicalMaterial（transmission/dispersion/DoubleSideの内部反射）＋宝石ショーケース専用環境マップ＋Bloomで、傾けるとファセットがきらめく。実物の屈折率・分散をデータ化。Phase 1=先頭5石+オパール
