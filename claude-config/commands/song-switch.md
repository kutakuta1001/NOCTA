---
description: "別の楽曲ブランチに作業を切り替えたいとき、または /song-switch [曲名] を実行するとき"
---
...
## Gotchas
- 未コミットの変更がある場合は `git stash` を実行してから `git checkout` する（変更を失わない）
- `song/[曲名]` ブランチが存在しない場合はエラーメッセージを表示して abort する
- NuWord は `song/` ブランチではなく main ブランチで管理している（切り替え不要）
