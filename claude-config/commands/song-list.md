---
description: "全楽曲ブランチのフェーズ・進捗状況を一覧で確認したいとき、または /song-list を実行するとき"
---

現在の全楽曲ブランチのステータスを一覧表示してください。

以下の手順で実行してください:

1. `git branch --list "song/*"` で `song/` から始まるブランチを一覧取得する
2. `git branch --show-current` で現在チェックアウト中のブランチ名を取得し、★マークで表示する
3. 各ブランチについて以下の情報を取得する:
   - `git show song/[曲名]:context.md` で「現在のフェーズ」を読み取る
   - `git show song/[曲名]:handoff.md` の最終行（直近の状態）を取得する
   - `git log -1 --format="%ar" song/[曲名]` で最終更新日時を取得する
4. 以下のフォーマットで表示する:

```
┌─────────────┬───────────────┬──────────────────────────────┬────────────┐
│ 曲名        │ フェーズ      │ 直近の状態                   │ 最終更新   │
├─────────────┼───────────────┼──────────────────────────────┼────────────┤
│ ★NuWord     │ フェーズ2-C   │ SVP生成済み。ゲート⑤待ち     │ 2時間前    │
│  NextSong   │ フェーズ1     │ トレンド分析中               │ 3日前      │
└─────────────┴───────────────┴──────────────────────────────┴────────────┘
★ = 現在作業中
```

`song/` ブランチが存在しない場合は:
「楽曲プロジェクトが見つかりません。/music-init [曲名] で開始してください」
と表示してください。

## Gotchas
- `song/` ブランチが存在しない場合は案内メッセージを表示して正常終了（エラーにしない）
- NuWord は main ブランチで管理しており `song/NuWord` ブランチは存在しない（メモリに記録済み）
