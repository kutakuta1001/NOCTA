...
**アーカイブとの照合（必須）:** `drafts/best-practices-archive.md` と
`best-practices-archive-xmcp.md`（過去の処理済み）に同じ知見・同じURLがないか確認する。
全文 Read は重いので、各エントリの固有キーワード（機能名・環境変数名・バージョン番号）で grep する。
一致した場合もエントリは落とさず、レポート冒頭に「## 収集の取りこぼし」節を設けて
「何回目の収集か」「CLAUDE.md に反映済みか」を明記する。
過去に収集したが未反映の項目は**優先度を1段上げて提案する**（再収集のコストが実際に発生しているため）。
2026-08-21 の実績では30件中4件が再収集で、うち1件は3回目だった。
