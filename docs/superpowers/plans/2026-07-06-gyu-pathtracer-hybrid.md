...
（スパイク確定: three-gpu-pathtracer@0.0.24 + three-mesh-bvh@0.9.10 が three@0.180.0 互換。
three-gpu-pathtracer が内部で `three/examples/jsm/` を絶対import するため、その prefix エントリも必須。）
...
### Task 5: 現像を美しくする（emissive実光源＋PT環境で内部干渉模様を出す）

これがPTの見た目の価値を実現する中核。**スパイクで判明した2つの事実に基づき計画を修正**:
(1) three-gpu-pathtracer は `MeshBasicMaterial`（既存ショーケースのライトカード）を発光と認識しない。
    `material.emissive * material.emissiveIntensity` のみを光源として拾う。
(2) `scene.environment` に入っている PBR用 PMREM texture は PT が equirect HDR として読もうとして例外を投げる
    （Task 4 は `withEnvOff` で develop 中だけ env を null 退避して回避している）。

よってPT用に、emissive な `MeshStandardMaterial` 光源群と、手続き的な PT 環境（`GradientEquirectTexture`）を用意し、
Task 4 の `withEnvOff`（env=null 退避）を「PT専用環境に差し替え」へ置き換える。宝石内部の多重反射・干渉模様は
これらの実光源＋環境が宝石を通して屈折することで生まれる。

**Files:**
- Modify: `website/gems/gem3d.js`（PT用光源群・PT環境の生成、develop/pbr 切替、detach 解放）
- Modify: `website/gems/gem3d-pathtracer.js`（`createDeveloper` に `environment` を受け取り setScene 前に PT用sceneへ適用）
- Test: `/private/tmp/.../scratchpad/verify-gem3d-hybrid.js` 更新 + `verify-pt-develop-shot.js`（現像スクショ）

**Interfaces:**
- Consumes: `GradientEquirectTexture`（three-gpu-pathtracer 同梱）、既存 `buildShowcaseScene` のライト配置知識。
- Produces:
  - gem3d.js に `buildEmissiveLightGroup() -> THREE.Group`（`MeshStandardMaterial` に `emissive`＋`emissiveIntensity` を持つ板を、ショーケースと同レイアウト・同輝度感で配置）。PBR時 `visible=false`、develop時 `visible=true`。
  - PT環境テクスチャ（`GradientEquirectTexture`：上=暖色明・下=暗）を develop 中だけ `scene.environment` に差す（`withEnvOff` を「savedEnv退避→PT環境を差す→finallyでsavedEnv復帰」に拡張）。

- [ ] **Step 1: emissive光源が拾えることを確認するテスト**

`verify-gem3d-hybrid.js` に、develop 到達後 scene 内に `isMeshStandardMaterial` かつ `emissiveIntensity>0` のメッシュが
3つ以上あり、かつ PBR復帰で `visible=false` になることを確認するチェックを追記。
Run: `node verify-gem3d-hybrid.js`
Expected: FAIL（emissive光源群 未実装）

- [ ] **Step 2: buildEmissiveLightGroup を実装**

```js
/* PT用の実光源群。three-gpu-pathtracer は emissive(MeshStandardMaterial) のみ光源として拾う
   （既存 buildShowcaseScene の MeshBasicMaterial は PT では発光しない）。ショーケースと似た配置で。 */
function buildEmissiveLightGroup() {
  ...
  specs.forEach(function (s) {
    var m = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: 0x000000, emissive: new THREE.Color(s.color), emissiveIntensity: s.intensity, side: THREE.DoubleSide })
    );
...
`mount()` 内で生成し scene に add、初期 `visible=false`:

```js
var ptLightGroup = buildEmissiveLightGroup();
ptLightGroup.visible = false;
scene.add(ptLightGroup);
```

- [ ] **Step 3: PT環境テクスチャを用意し withEnvOff を差し替え**

```js
// mount内。GradientEquirectTexture は three-gpu-pathtracer から import できないため（gem3d.jsはthreeのみ）、
// gem3d-pathtracer.js 側で環境を生成して developer に持たせる方式にする（下記 Step 4）。
```

gem3d.js の `withEnvOff` を「PT環境へ差し替え」に拡張し、develop 中は PT光源群を可視化:

```js
function withPtEnv(fn) {
  var savedEnv = scene.environment;
  scene.environment = null;               /* PMREMを外す（PTは develop 側の環境/emissiveで照らす） */
  ptLightGroup.visible = true;            /* PT光源を出す */
  try { return fn(); }
  finally { scene.environment = savedEnv; ptLightGroup.visible = false; }  /* PBR用に即復帰 */
}
```

- `beginDevelop` / `resync` 呼び出しの `withEnvOff(...)` を `withPtEnv(...)` に置換。
- ただし PT はレンダリング中ずっと光源が要る。develop ループ中は `ptLightGroup.visible=true` を維持し、
  `switchToPbr` で `false` に戻す設計に修正する（`withPtEnv` は setScene/resync の瞬間の env 差し替え用。
  光源可視は develop 状態と連動させる）。具体的には:
  - `beginDevelop` 成功時（developer生成/reuse後）: `ptLightGroup.visible = true`
  - `switchToPbr`: `ptLightGroup.visible = false`
  - loop の develop 分岐は PBR を描かないので、PBR見た目への影響はなし。

- [ ] **Step 4: gem3d-pathtracer.js に PT環境を持たせる**

`createDeveloper` で PT用の `GradientEquirectTexture` を生成し、setScene 前に `scene.environment` に設定してから
setScene、直後に復帰（PTは setScene 時に環境を取り込む）。または three-gpu-pathtracer の
`pathTracer.setScene(scene, camera)` 前に一時的に `scene.environment = ptEnv` にする:

```js
import { WebGLPathTracer, GradientEquirectTexture } from 'three-gpu-pathtracer';
// createDeveloper 内:
var ptEnv = new GradientEquirectTexture();
ptEnv.topColor.set(0x3a3630); ptEnv.bottomColor.set(0x0a0906); ptEnv.update();
function applyScene() {
  var saved = scene.environment;
  scene.environment = ptEnv;
  try { pt.setScene(scene, camera); } finally { scene.environment = saved; }
}
// reset/resync/初期化は applyScene() を通す。dispose で ptEnv.dispose()。
```

（これにより gem3d.js 側の env 退避は「PMREMを外す」だけでよくなり、PT の環境は developer が自前で持つ。
gem3d.js の withPtEnv は「PMREMを外す＋PT光源可視化」に単純化できる。実装が二重管理にならないよう、
env の主管理は developer 側（GradientEquirect）、gem3d 側は PMREM の一時退避と光源可視化のみ、と役割を分ける。）

- [ ] **Step 5: テスト＋現像スクショ**

Run: `node verify-gem3d-hybrid.js`（emissive光源群がdevelop時visible・PBR時非visible、状態遷移維持、12項目級 PASS）
Run: `node verify-pt-develop-shot.js`（新規：ダイヤを開き develop を最大まで走らせPT canvasをスクショ `pt-develop-diamond.png`）
Expected: 現像後のダイヤに内部の多重反射・干渉模様が出る（暗背景＋実光源）。コンソールエラー0。
既存回帰: `node verify-gem3d.js` 11/11 維持。

- [ ] **Step 6: detach で PT光源群・PT環境を解放**

`detach()` に `ptLightGroup` の traverse dispose を追加（geometry/material）。PT環境は developer.dispose() 側で解放。

- [ ] **Step 7: Commit**

```bash
cd /Users/fghmacbook013/NOCTA/project_NOCTA
git add website/gems/gem3d.js website/gems/gem3d-pathtracer.js
git commit -m "feat(gems): 現像をemissive実光源＋PT環境で照らし内部干渉模様を出す"
```
