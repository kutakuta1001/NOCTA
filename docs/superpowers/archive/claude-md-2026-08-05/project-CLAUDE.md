...
- Agent Teams起動時は全員Sonnet 5で開始し、レビューフェーズでOpus 5に切り替える
- **Opus 5 切替通知ルール（デフォルトはSonnet 5）**: 以下のタイミングになったら作業を始める前に「Opus 5 への切り替えを推奨します: `/model claude-opus-5`」と通知し、CEOの確認を待つ。切り替え不要と言われたらSonnet 5のまま続ける。
